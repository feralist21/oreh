...
if ($act == 'callback') {
  $name = isset($_POST['modal-name']) ? strip_tags($_POST['modal-name']) : '';
  $phone = isset($_POST['modal-phone']) ? strip_tags($_POST['modal-phone']) : '';
  $theme = 'Заявка на обратный звонок';
  $message .= "Поступила новая заявка на обратный звонок с сайта mebel-v-dom.com\n\n";
  $message .= "Имя: $name\n";
  $message .= "Телефон: $phone\n";
} else if ($act == 'callback-zam') {
    ...
    $theme = 'Заявка на замер шкафа';
    $message .= "Поступила новая заявка на заказ замеров шкафа с сайта mebel-v-dom.com\n\n";
    $message .= "Имя: $name\n";
    $message .= "Телефон: $phone\n";
